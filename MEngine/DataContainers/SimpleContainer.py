...
class SimpleContainer:
    ...
    def find_object(self, data_object: object) -> str or int or float or tuple or None:
        '''
        Функция првоеряет, есть ли указанный объект в наборе, и отдаёт его индекс, если таковой имеется.

        :param data_object: объект, который ищем
        :return: индекс в указанном формате или None
        '''
        if not isinstance(data_object, self.objects_type):  # Если тип объекта неверный
            return None

        for index in self.__objects_dict.keys():
            if self.__objects_dict[index] == data_object:
                return index
        return None
